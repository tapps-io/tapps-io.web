...
const createApolloClient = (initialState: NormalizedCacheObject, ctx?: NextPageContext) => {
  ...
  return new ApolloClient({
    ssrMode: Boolean(ctx),
    link: new HttpLink({
      uri: `${process.env.CMS}/graphql`, // Server URL (must be absolute)
 ...
 */
export const withApollo = <PageProps, InitialProps = PageProps>(PageComponent: NextPage<PageProps, InitialProps>) => ({
  ssr = false,
} = {}) => {
  ...
  if (ssr || PageComponent.getInitialProps) {
    WithApollo.getInitialProps = async (ctx: NextPageContext | AppContext) => {
      ...
      let pageProps: any = {};
      if (isAppContext(ctx)) {
        pageProps = (await App.getInitialProps(ctx)).pageProps;
      ...
      if (typeof window === 'undefined') {
        ...
        if (!isAppContext(ctx) && ctx.res && ctx.res.finished) {
          return pageProps;
        }
